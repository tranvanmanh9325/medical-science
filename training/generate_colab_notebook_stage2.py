...
"""
    ...
    DRIVE_CELL = [
        "# Step 2: Clone GitHub repo (for checkpoint Git push)\n",
        "import os, subprocess\n",
        "GITHUB_TOKEN = ''  # <-- Paste your GitHub token here for auto-push\n",
        "os.environ['GITHUB_TOKEN'] = GITHUB_TOKEN\n",
        "if GITHUB_TOKEN and not os.path.exists('/content/medical-science'):\n",
        "    repo_url = f'https://x-access-token:{GITHUB_TOKEN}@github.com/tranvanmanh9325/medical-science.git'\n",
        "    subprocess.run(['git', 'clone', '--depth=1', repo_url, '/content/medical-science'], check=True)\n",
        "    print('[OK] Repo cloned')\n",
        "else:\n",
        "    print('[INFO] No GITHUB_TOKEN - checkpoints saved to /content/checkpoints only')"
    ]
    ...
    nb = {
        ...
        "metadata": {
            "accelerator": "GPU",
            "colab": {"gpuType": "T4", "provenance": [], "name": "Apollo Humanoid Stage 2 Walking v8"},
    ...
    print(f"[COLAB STAGE 2 v8] {nb_path} ({os.path.getsize(nb_path) // 1024}KB)")
    return nb_path
